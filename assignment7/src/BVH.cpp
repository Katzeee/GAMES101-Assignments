...
BVHBuildNode *BVHAccel::recursiveBuild(std::vector<Object *> objects) {
  ...
  Bounds3 bounds;
  for (int i = 0; i < objects.size(); ++i) bounds = Union(bounds, objects[i]->getBounds());
  if (objects.size() == 1) {
    ...
    for (int i = 0; i < objects.size(); ++i) centroidBounds = Union(centroidBounds, objects[i]->getBounds().Centroid());
...
Intersection BVHAccel::getIntersection(BVHBuildNode *node, const Ray &ray) const {
  ...
  Intersection isect;
  std::queue<BVHBuildNode *> q;
  q.push(node);
  BVHBuildNode *tmp_node;
  while (!q.empty()) {
    tmp_node = q.front();
    q.pop();
    ...
    if (tmp_node->object != nullptr) {
      auto tmp_isect = tmp_node->object->getIntersection(ray);
