...
static bool insideTriangle(int x, int y, const Vector4f *_v) {
  ...
  f2 = v[0].cross(v[2]);
  Vector3f p(x, y, 1.);
  if ((p.dot(f0) * f0.dot(v[2]) > 0) && (p.dot(f1) * f1.dot(v[0]) > 0) && (p.dot(f2) * f2.dot(v[1]) > 0)) return true;
  return false;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
  ...
  // Use: fragment_shader_payload payload( interpolated_color, interpolated_normal.normalized(), interpolated_texcoords,
  // texture ? &*texture : nullptr); Use: payload.view_pos = interpolated_shadingcoords; Use: Instead of passing the
  // triangle's color directly to the frame buffer, pass the color to the shaders first to get the final color; Use:
